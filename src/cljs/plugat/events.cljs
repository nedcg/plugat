...
(re-frame/reg-event-db
	::set-post-payload
	(fn [db [_ plug-id payload]]
		(update db :post-payload assoc plug-id payload)))
...
(re-frame/reg-event-fx
	::post-payload!
	(fn [{{auth                :auth
				 payload-map         :post-payload
				 {plug-id :plugs/id} :current-plug} :db} _]
		{:http-xhrio {:method           :put
									:uri              (str "http://localhost:3000/api/plugs/" plug-id "/messages")
									:headers          {"authorization" (str "Bearer " (auth/token auth))}
									:params           {:payload (get payload-map plug-id)}
									:timeout          8000
									:format           (ajax/json-request-format)
									:response-format  (ajax/transit-response-format {:keywords? true})
									:with-credentials true
									:on-success       [::set-post-payload plug-id ""]
									:on-failure       [::print-error]}}))
